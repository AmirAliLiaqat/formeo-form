...
(function(global2, factory) {
    ...
    processOptions(options, elem, isPreview) {
      ...
      const optionMap = (option, i2) => {
        ...
        const optionMarkup = {
          ...
          button: (option2) => {
            const { type, label: label2, className, id: id2 } = option2;
            return {
              ...elem,
              attrs: {
                type
  ...
  const makeComponentOptionsList = (component, autocomplete) => {
    const items = component.data.options.map((option, index2) => {
    ...
    boolean: (dataKey, field2) => ({
      click: ({ target }) => {
        if (target.type === "radio") {
          const updatedOptions = field2.data.options.map((option) => ({ ...option, selected: false }));
  ...
  const FormeoEditor = FormeoEditor$1;
  const FormeoRenderer = FormeoRenderer$1;
  const columnControl = {
    config: {
      label: "column"
    },
  ...
  exports2.FormeoEditor = FormeoEditor;
  exports2.FormeoRenderer = FormeoRenderer;
